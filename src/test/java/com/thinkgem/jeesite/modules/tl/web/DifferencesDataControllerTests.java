...
public class DifferencesDataControllerTests extends BaseControllerTests {
	@Autowired
	DifferencesDataController differencesDataController;
	...
	public void setup() {
		// 设置测试Controller
		setupController(differencesDataController);
	...
	public void getSave() throws Exception { 
		String url = "/a/tl/differencesData/save";
		DifferencesData differencesData = new DifferencesData();
		//differencesData.setId("1");//自定义id
		//differencesData.setIsNewRecord(true);// 修改默认为新对象的标识，和自定义id配合使用
		//differencesData.setName("测试内容");
		 

		// 发送POST请求
		ResultActions resultAction = post(url, JsonMapper.toJsonString(differencesData));// "{\"id\":\"1\",\"name\":\"测试内容\"}"
		// 返回200
	...
	public void get() throws Exception {
		String url = "/a/tl/differencesData/get";
		// 发送GET请求
