...
public class JobUserControllerTests extends BaseControllerTests {
	@Autowired
	JobUserController jobUserController;
	...
	public void setup() {
		// 设置测试Controller
		setupController(jobUserController);
	...
	public void getSave() throws Exception { 
		String url = "/a/tl/jobUser/save";
		JobUser jobUser = new JobUser();
		//jobUser.setId("1");//自定义id
		//jobUser.setIsNewRecord(true);// 修改默认为新对象的标识，和自定义id配合使用
		//jobUser.setName("测试内容");
		 

		// 发送POST请求
		ResultActions resultAction = post(url, JsonMapper.toJsonString(jobUser));// "{\"id\":\"1\",\"name\":\"测试内容\"}"
		// 返回200
