...
public class JobControllerTests extends BaseControllerTests {
	@Autowired
	...
	public void getSave() throws Exception { 
		String url = "/a/tl/job/save";
		Job job = new Job();
		//job.setId("1");//自定义id
		//job.setIsNewRecord(true);// 修改默认为新对象的标识，和自定义id配合使用
		//job.setName("测试内容");
		 

		// 发送POST请求
		ResultActions resultAction = post(url, JsonMapper.toJsonString(job));// "{\"id\":\"1\",\"name\":\"测试内容\"}"
		// 返回200
