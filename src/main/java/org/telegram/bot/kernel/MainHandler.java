 ...
 * @date 13.001.14
 */
@SuppressWarnings({ "OverlyComplexClass", "OverlyLongMethod",
		"OverlyComplexMethod", "InfiniteLoopStatement",
...
public class MainHandler implements NotificationsService.NotificationObserver {
	...
	public MainHandler(IKernelComm kernelComm, UpdatesHandlerBase updatesHandler) {
	...
	/**
	 * Add a TLAbsUpdates to the updates queue. Load Updates states from
	 * database if needed
	 * 
	 * @param updates
	 *            Updates to add
	...
	private void onTLAbsUpdates(@NotNull final TLAbsUpdates updates) {
		...
		if (updates instanceof TLUpdateShortMessage) {
			...
			wrapper.setParams(updateShortMessage.getPts(),
					updateShortMessage.getPtsCount(),
			...
			final UpdateWrapper wrapper = new UpdateWrapper(
					updateShortChatMessage);
			wrapper.setParams(updateShortChatMessage.getPts(),
					updateShortChatMessage.getPtsCount(),
			...
			final UpdateWrapper wrapper = new UpdateWrapper(
					updateShort.getUpdate());
			wrapper.setParams(updateShort.getUpdate().getPts(), updateShort
					.getUpdate().getPtsCount(), updateShort.getDate(), 0, 0);
			updateHandlerThread.addUpdate(wrapper);
			...
			if (disablePtsCheck) {
				correctSeq = updatesHandler.checkSeq(tlUpdates.getSeq(), 0,
						tlUpdates.getDate());
			}
			...
			boolean correctSeq = true;
			if (disablePtsCheck) {
				correctSeq = updatesHandler.checkSeq(updatesCombined.getSeq(),
						updatesCombined.getSeqStart(),
						updatesCombined.getDate());
			}

			if (correctSeq) {
				updatesHandler.onChats(updatesCombined.getChats());
				updatesHandler.onUsers(updatesCombined.getUsers());
				updatesCombined.getUpdates().forEach(
						x -> {
							final UpdateWrapper wrapper = new UpdateWrapper(x);
							wrapper.setParams(x.getPts(), x.getPtsCount(),
									updatesCombined.getDate(),
									updatesCombined.getSeq(),
									updatesCombined.getSeqStart());
							if (disablePtsCheck) {
								wrapper.disablePtsCheck();
							}
			...
			wrapper.setParams(updateShortSentMessage.getPts(),
					updateShortSentMessage.getPtsCount(),
			...
			BotLogger.debug(LOGTAG,
					"Unsupported TLAbsUpdates: " + updates.toString());
		}
	}
	...
	private void getUpdatesState() {
		try {
			final TLUpdatesState state = kernelComm
					.doRpcCallSync(new TLRequestUpdatesGetState());
			if (state != null) {
	...
	private class UpdateHandlerThread extends Thread {
		...
		private final PriorityQueue<UpdateWrapper> updates = new PriorityQueue<>(
				new UpdateWrapper.UpdateWrapperComparator());
