...
public class RegTelegramBot extends TelegramBot {
    ...
    public RegTelegramBot(BotConfig config, ChatUpdatesBuilder chatUpdatesBuilder, int apiKey, String apiHash) {
...
//        this.kernelAuth = new KernelAuth(this.apiState, config, this.kernelComm, apiKey, apiHash);
        this.kernelAuth = new RegKernelAuth(this.apiState, config, this.kernelComm, apiKey, apiHash);
        BotLogger.info(LOGTAG, String.format("%s init in %d ms", this.kernelAuth.getClass().getName(), (start - System.currentTimeMillis()) * -1));
    }
