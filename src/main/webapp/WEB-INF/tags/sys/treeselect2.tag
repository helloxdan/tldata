	...
	<input id="${id}Name" name="${labelName}" ${allowInput?'':'readonly="readonly"'} type="text" value="${labelValue}" data-msg-required="${dataMsgRequired}"
		class="form-control ${cssClass}" style="${cssStyle}"/>
		 <span class="input-group-btn">
            <button type="button" id="${id}Button" class="btn btn-flat ${disabled} ${hideBtn ? 'hide' : ''}" style="${smallBtn?'padding:4px 2px;':''}"><span class="glyphicon glyphicon-folder-close" style="color: #bababa;"></span></button>
         </span>
</div>
