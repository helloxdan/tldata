...
<script type="text/javascript">
	var key, lastValue = "", nodeList = [], type = getQueryString("type",
			"${url}");
	var isbig=getQueryString("isbig","") || 0; 
	var tree, setting = {
		view : {
			selectedMulti : true,
			dblClickExpand : true,
		...
		check : {
			enable : '${checked}',
			nocheckInherit : true,
			chkStyle: '${checked}'=='false'?"radio":"checkbox"
		...
		callback : {
			...
			onExpand: loadChildNode,
			onClick :loadChildNode,
	...
	var treeMap={};
	 function loadTree(type) {
		 ...
		 if(treeMap[type]){
			 tree=treeMap[type]
			 console.log(type+'树已存在');
			 return;
		 }
		 var url = "${ctx}/tl/group/tree?type="+type+"&pId=0&isTop=true&nodeType=folder"
				$.get(url + "&t=" + new Date().getTime(),
						function(zNodes) {
						zNodes=zNodes.data.list;
						console.log('初始化树……'+type);
							// 初始化树结构
							...
							//异步加载子节点（加载用户）
							var nodesOne = tree.getNodesByParam(
									"isParent", true);
							for (var j = 0; j < nodesOne.length; j++) {
								tree.reAsyncChildNodes(nodesOne[j],
										"!refresh", true);
							}
							selectCheckNode();
						});
	 ...
	 function loadChildNode(event, treeId, treeNode) {
		 
	 		if(type=='myasset' || type=='mystar' )
	 			return;
	 		
			//tree.expandNode(treeNode);
			var id = treeNode.id;// treeNode.pId == '0' ? '' :treeNode.pId;
			var pIds = treeNode.pIds + id + ',';

			if (treeNode.nodeType == 'folder' || treeNode.nodeType == 'type') {
				if (treeNode.dataloaded) {
					//折叠
				} else {
					treeNode.dataloaded = true;
					loading('数据加载中……');
					//点击设备类型
					$.getJSON("${ctx}/tl/group/tree?nodeType=folder&type="+type+"&pId=" + id ,
							function(data) {
								//console.log('type tree data:',data);
								if (data && data.data.list.length > 0)
									tree.addNodes(treeNode, data.data.list, false);
				...
				//$('#assetContent').attr("src","${ctx}/icareplus/asset/form2?id="+id);
			}
		}
